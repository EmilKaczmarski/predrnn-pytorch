...
class RNN(nn.Module):
    def __init__(self, num_layers, num_hidden, configs):
        ...
        self.conv_last = nn.Conv2d(num_hidden[num_layers - 1], self.frame_channel, kernel_size=1, stride=1, padding=0,
                                   bias=False)
        self.adapter = nn.Conv2d(num_hidden[0], num_hidden[0], 1, stride=1, padding=0, bias=False)

    def forward(self, frames_tensor, mask_true):
        frames = frames_tensor.permute(0, 1, 4, 2, 3).contiguous().half()
        mask_true = mask_true.permute(0, 1, 4, 2, 3).contiguous().half()
        ...
        for i in range(self.num_layers):
            zeros = torch.zeros([batch, self.num_hidden[i], height, width], dtype=torch.float16).to(self.configs.device)
            h_t.append(zeros)
            c_t.append(zeros)
            delta_c_list.append(zeros)
            delta_m_list.append(zeros)

        memory = torch.zeros([batch, self.num_hidden[0], height, width], dtype=torch.float16).to(self.configs.device)
        ...
        loss = self.MSE_criterion(next_frames.float(), frames_tensor[:, 1:].float()) + self.configs.decouple_beta * decouple_loss
